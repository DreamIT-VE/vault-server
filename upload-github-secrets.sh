...
# Usage:
#   ./upload-github-secrets.sh -f .env -r DreamIT-VE/vault-server -e production

set -euo pipefail
shopt -s extglob  # enable +() patterns used for trimming

ENV_FILE=".env"
REPO="DreamIT-VE/vault-server"
...
Options:
  ...
  -r  GitHub repo in owner/name format (default: DreamIT-VE/outline-server)
